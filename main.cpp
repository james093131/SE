...
int main(int argc, const char * argv[]) {
    ...
    int run = atoi(argv[2]);//run
    int Bit = atoi(argv[3]);//length
    ...
    SE_ini.SampleV.resize(Searcher_Quan*Region*Region*Good_Quan,vector<int>(Bit));//Searcher*Good and legth is bits
    SE_ini.SampleV_Fitness.resize(Searcher_Quan*Region*Region*Good_Quan);//How many 1 in the SampleV
    SE_ini.Region_Fitness.resize(Region,vector<int>(Good_Quan,-100));//各區最好的前Sample個 
    SE_ini.T_Visit.resize(Region,1);//各區的造訪情形
    SE_ini.T_Not_Visit.resize(Region,1);
    //SE_ini initial

    Vision_Search CLC_Expected_Value;//計算expected Value 用的
    CLC_Expected_Value.SampleV_Region_Fitness.resize(Region); //各區的Fitness加總
    CLC_Expected_Value.Region_Best_Fitness.resize(Region);//各區中最好的那條
    CLC_Expected_Value.Visit_Or_Not.resize(Region);//T_Not_Visit/T_Visit
    CLC_Expected_Value.Expected_FitnessValue.resize(Region);//計算完後儲存Expected Value用的
    //CLC_Expected_Value initial 

    Final FIN_INF;
    FIN_INF.Best_Searcher.resize(Bit);//儲存此ＩＴＥＲ中的最佳解
    //Final information initial

    RUN Runresult;
    Runresult.AVG_Iter_Searcher_Fitness.resize(ITE);//儲存當Run的最佳解
    Runresult.AVG_Best_Search_Fitness=0;//儲存當Run的最佳解Fitness
    ...
    while(r<run)
    {
        ...
        random_zero_or_one(SE_ini.Searcher);//0,1隨機起始
        ...
        CLC_Expected_Value.SampleV_Region_Fitness = Region_fitness(SE_ini.Good,SE_ini.SampleV,SE_ini.SampleV_Fitness,SE_ini.Region_Fitness,Region,Searcher_Quan*Good_Quan*Region);
    ...
    //     }
        ...
        Marketing_Research(SE_ini.Searcher,CLC_Expected_Value.Expected_FitnessValue,SE_ini.Good,SE_ini.T_Visit,SE_ini.T_Not_Visit,Region);
    ...
    InformationOutput(run,Bit,Region,Searcher_Quan*Region,Good_Quan*Region,ITE*Region*Region*Searcher_Quan*Good_Quan,Runresult.AVG_Best_Search_Fitness,Runresult.AVG_Iter_Searcher_Fitness,START,END);
